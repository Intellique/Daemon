...
static void * solgr_loader_load_file(const char * filename) {
	if (access(filename, R_OK | X_OK)) {
		solgr_log_write2(so_log_level_debug, so_log_type_logger, dgettext("libstoriqone", "Loader: access to file %s failed because %m"), filename);
		return NULL;
	}
	...
	if (cookie == NULL) {
		solgr_log_write2(so_log_level_debug, so_log_type_logger, dgettext("libstoriqone", "Loader: failed to load '%s' because %s"), filename, dlerror());
