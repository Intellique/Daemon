...
static void * so_loader_load_file(const char * filename) {
	if (access(filename, R_OK | X_OK)) {
		so_log_write(so_log_level_debug, dgettext("libstoriqone", "libstoriqone: loader: failed to access to file '%s' because %m"), filename);
		return NULL;
	}
	...
	if (cookie == NULL) {
		so_log_write(so_log_level_debug, dgettext("libstoriqone", "libstoriqone: loader: failed to load '%s' because %s"), filename, dlerror());
