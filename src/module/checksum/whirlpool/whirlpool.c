...
*  Last modified: Thu, 01 Mar 2012 12:57:51 +0100                         *
\*************************************************************************/
...
static struct st_checksum * st_checksum_whirlpool_clone(struct st_checksum * new_checksum, struct st_checksum * current_checksum);
...
static struct st_checksum_ops st_checksum_whirlpool_ops = {
	.clone	= st_checksum_whirlpool_clone,
...
struct st_checksum * st_checksum_whirlpool_clone(struct st_checksum * new_checksum, struct st_checksum * current_checksum) {
	if (!current_checksum)
		return 0;

	struct st_checksum_whirlpool_private * current_self = current_checksum->data;

	if (!new_checksum)
		new_checksum = malloc(sizeof(struct st_checksum));

	new_checksum->ops = &st_checksum_whirlpool_ops;
	new_checksum->driver = &st_checksum_whirlpool_driver;

	struct st_checksum_whirlpool_private * new_self = malloc(sizeof(struct st_checksum_whirlpool_private));
