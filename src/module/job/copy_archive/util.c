...
bool st_job_copy_archive_select_output_media(struct st_job_copy_archive_private * self) {
	...
	if (unbreakable_level == st_pool_unbreakable_level_archive && self->archive_size > self->pool->format->capacity) {
		...
		st_job_add_record(self->job->db_connect, st_log_level_warning, self->job, st_job_record_notif_important, "Fatal error, archive's size (%s) is greater than a media (media size: %s) and the unbreakable level is archive", buf_archive_size, buf_media_size);
		return false;
	}
	...
	while (!stop) {
		switch (state) {
			case check_offline_free_size_left:
				total_size += self->job->db_connect->ops->get_available_size_of_offline_media_from_pool(self->job->db_connect, self->pool);

				if (self->archive_size - self->total_done > total_size) {
					// alert user
					if (!has_alerted_user)
						st_job_add_record(self->job->db_connect, st_log_level_warning, self->job, st_job_record_notif_important, "Please, insert media which is a part of pool named %s", self->pool->name);
			...
			case has_media:
				if (self->drive_output->slot->media == NULL) {
					...
					st_job_add_record(self->job->db_connect, st_log_level_info, self->job, st_job_record_notif_normal, "Loading media (%s) from slot #%td to drive #%td of changer [ %s | %s ]", media->name, slot - changer->slots, self->drive_output - changer->drives, changer->vendor, changer->model);

					int failed = changer->ops->load_slot(changer, slot, self->drive_output);
					slot->lock->ops->unlock(slot->lock);

					if (failed) {
						st_job_add_record(self->job->db_connect, st_log_level_error, self->job, st_job_record_notif_important, "Loading media (%s) from slot #%td to drive #%td of changer [ %s | %s ] finished with code = %d", media->name, slot - changer->slots, self->drive_output - changer->drives, changer->vendor, changer->model, failed);
						return false;
					} else
						st_job_add_record(self->job->db_connect, st_log_level_info, self->job, st_job_record_notif_normal, "Loading media (%s) from slot #%td to drive #%td of changer [ %s | %s ] finished with code = OK", media->name, slot - changer->slots, self->drive_output - changer->drives, changer->vendor, changer->model);
				}
			...
			case has_wrong_media:
				...
				if (self->drive_output->slot->media != NULL && self->drive_output->slot != slot) {
					...
					st_job_add_record(self->job->db_connect, st_log_level_info, self->job, st_job_record_notif_normal, "Unloading media (%s) from drive #%td of changer [ %s | %s ]", media->name, self->drive_output - changer->drives, changer->vendor, changer->model);

					int failed = changer->ops->unload(changer, self->drive_output);
					if (failed) {
						st_job_add_record(self->job->db_connect, st_log_level_error, self->job, st_job_record_notif_important, "Unloading media (%s) from drive #%td of changer [ %s | %s ] finished with code = %d", media->name, self->drive_output - changer->drives, changer->vendor, changer->model, failed);
						return false;
					} else
						st_job_add_record(self->job->db_connect, st_log_level_info, self->job, st_job_record_notif_normal, "Unloading media (%s) from drive #%td of changer [ %s | %s ] finished with code = OK", media->name, self->drive_output - changer->drives, changer->vendor, changer->model);
				}
			...
			case is_media_formatted:
				if (self->drive_output->slot->media->status == st_media_status_new) {
					...
					st_job_add_record(self->job->db_connect, st_log_level_info, self->job, st_job_record_notif_normal, "Formatting new media (%s) from drive #%td of changer [ %s | %s ]", media->name, self->drive_output - changer->drives, changer->vendor, changer->model);

					int failed = st_media_write_header(self->drive_output, self->pool, self->job->db_connect);
					if (failed) {
						st_job_add_record(self->job->db_connect, st_log_level_error, self->job, st_job_record_notif_important, "Formatting new media (%s) from drive #%td of changer [ %s | %s ] finished with code = %d", media->name, self->drive_output - changer->drives, changer->vendor, changer->model, failed);
						return false;
					} else
						st_job_add_record(self->job->db_connect, st_log_level_info, self->job, st_job_record_notif_normal, "Formatting new media (%s) from drive #%td of changer [ %s | %s ] finished with code = OK", media->name, self->drive_output - changer->drives, changer->vendor, changer->model);
				}
			...
			case media_is_read_only:
				if (self->drive_output->slot->media->type == st_media_type_readonly) {
					st_job_add_record(self->job->db_connect, st_log_level_warning, self->job, st_job_record_notif_important, "Media '%s' is currently read only ", self->drive_output->slot->media->name);
