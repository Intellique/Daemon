...
*  Last modified: Fri, 30 Nov 2012 17:44:56 +0100                         *
\*************************************************************************/
...
int st_job_save_run(struct st_job * job) {
	...
	for (i = 0; i < jp->nb_tapes; i++) {
		jp->tapes[i]->locked = false;
		st_tape_sync(jp->tapes[i]);
	}
			...
			case is_pool_growable1:
				if (job->pool->growable) {
					...
					for (i = changer->nb_drives; i < changer->nb_slots && sl == NULL; i++) {
						...
						sl->tape->locked = true;
						st_tape_sync(sl->tape);
					}

					if (sl != NULL) {
						if (drive->slot != sl && drive->slot->tape != NULL) {
							drive->ops->eject(drive);
							changer->ops->unload(changer, drive);
						}
