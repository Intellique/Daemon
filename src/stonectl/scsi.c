...
static bool stctl_scsi_drive_in_changer2(int fd, bool barcode, int start_element, int nb_elements, struct st_value * drive);
...
bool stctl_scsi_drive_in_changer(struct st_value * changer, struct st_value * drive) {
	char * device = NULL;
	int barcode;
	if (st_value_unpack(changer, "{sssb}", "device", &device, "barcode", &barcode) < 2) {
		free(device);
		return false;
	}
	...
	bool ok = stctl_scsi_drive_in_changer2(fd, barcode, start_element, nb_elements, drive);
...
static bool stctl_scsi_drive_in_changer2(int fd, bool barcode, int start_element, int nb_elements, struct st_value * drive) {
	...
	char * dr_vendor = NULL, * dr_model = NULL, * dr_serial_number = NULL;
	if (st_value_unpack(drive, "{ssssss}", "vendor", &dr_vendor, "model", &dr_model, "serial number", &dr_serial_number) < 3) {
		free(dr_vendor);
		free(dr_model);
		return false;
	}
	...
	for (i = 0; i < result->number_of_elements && found == false; i++) {
		if (element_header->type == scsi_loader_element_type_data_transfer) {
			...
			dev[34] = '\0';
			memset(dev, ' ', 35);
			strncpy(dev, dr_vendor, strlen(dr_vendor));
			dev[7] = ' ';
			strncpy(dev + 8, dr_model, strlen(dr_model));
			dev[23] = ' ';
			strcpy(dev + 24, dr_serial_number);
