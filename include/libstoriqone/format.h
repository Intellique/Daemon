...
struct so_format_writer {
	struct so_format_writer_ops {
		enum so_format_writer_status (*add)(struct so_format_writer * fw, struct so_value * file);
		...
		int (*restart_file)(struct so_format_writer * fw, const char * filename, ssize_t position);
